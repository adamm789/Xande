...
public class MdlFileBuilder {
    ...
    public async Task<(MdlFile? file, List<byte> vertexData, List<byte> indexData)> BuildAsync() {
        ...
        var submeshTasks = new Dictionary<(int, int), Task<SubmeshBuilder>>();
        //var vertexDataTasks = new Dictionary<SubmeshBuilder, Dictionary<string, Task<Dictionary<int, List<byte>>>>>();
        //var meshVertexDataTasks = new Dictionary<LuminaMeshBuilder, Task<Dictionary<int, List<byte>>>>();
        ...
        for( var i = 0; i < _meshBuilders.Count; i++ ) {
            ...
            for( var j = 0; j < mesh.Submeshes.Count; j++ ) {
                ...
                foreach( var shapeName in submesh.Shapes ) {
                    //var values = await vertexDataTasks[submesh][shapeName];
        ...
        foreach( var mesh in _meshBuilders ) {
            vertexData.AddRange( await mvdTasks[mesh].GetBytesAsync() );
        }
